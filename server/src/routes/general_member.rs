...
use crate::{models::general_member::{GeneralMember, GeneralMemberRequest}, services::db::Database, utilities::admin_pagination_args::AdminPaginationArgs};
...
pub async fn get_all_general_members(db: Data<Database>, request: Json<AdminPaginationArgs>) -> HttpResponse {
  if !verify(request.token.clone(), hash(db.get_admin().await.unwrap().token.to_string(), DEFAULT_COST).unwrap().as_str()).unwrap() {
    return HttpResponse::Unauthorized().body("Unauthorized.");
  }

  match db.get_all_general_members(request.page, request.limit, request.search.clone(), request.field.clone()).await {
    Ok(members) => {
      let general_members: Vec<GeneralMemberRequest> = members.into_iter().rev().map(|member| GeneralMemberRequest {
